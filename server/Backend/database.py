...
def connect(*, read_only: bool = False) -> Connection:
    ...
    Do NOT use read-only mode to run untrusted, potentially malicious
...
def _create_and_connect() -> Connection:
    ...
    try:
        ...
        # the database wouldn't resolve a timeout, so re-raise the error.
        raise
    except psycopg.OperationalError:
        # Psycopg will raise an `OperationalError`, but it won't give an error
        # code, so we guess that creating the database will resolve the issue.
        _create()
        return _open_connection()
