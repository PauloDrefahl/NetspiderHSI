...
//changes the button label for select keysets, shows what keyset is selected
document.addEventListener('DOMContentLoaded', function () {
  var keysetLinks = document.querySelectorAll('.dropdown-item[data-keyset]');
  keysetLinks.forEach(function(link) {
      link.addEventListener('click', function(event) {
          event.preventDefault(); // Prevent the default action
          
          // Get the selected keyset from the data-keyset attribute
          var selectedKeyset = link.getAttribute('data-keyset');
          
          // Update the label text with the selected keyset
          var selectKeysetLabel = document.getElementById('select-keyset');
          selectKeysetLabel.textContent = "Selected keyset: " + selectedKeyset;
      });
  });
});
